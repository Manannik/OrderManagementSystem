    ...
    ILogger<CatalogController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> OrderProductQuantity([FromBody] OrderedQuantity request,
        CancellationToken ct)
    {
        logger.LogInformation("Вызван метод UpdateQuantity с id: {Id} и новым количеством товара {newQuantity}", request.Id,
            request.NewQuantity);
        ...
        logger.LogInformation("Для продукт с {id}, успешно обновлено количество товара", request.Id);
